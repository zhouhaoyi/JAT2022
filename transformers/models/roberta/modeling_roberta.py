...
class RobertaSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.use_jat and (not is_cross_attention) and (self.layer in self.olayers):
            use = self.num_attention_heads - self.oheads
            if use * 2 <= self.num_attention_heads:
...
class RobertaAttention(nn.Module):
    def __init__(self, config, layer):
        super().__init__()
        self.self = RobertaSelfAttention(config, layer)
...
class RobertaLayer(nn.Module):
    def __init__(self, config, layer):
        ...
        self.seq_len_dim = 1
        self.attention = RobertaAttention(config, layer)
        self.is_decoder = config.is_decoder
