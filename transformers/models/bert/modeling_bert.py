...
def get_attn_adj(scores, dim=-1, key=None, super_p=None, jat_sgn=False):
    A2 = scores.detach()
    ...
    if jat_sgn:
        #for solve -x * -x >= super_p
        sgn_mat = (A2 < 0).int()
        sgn_k_mat = torch.einsum("bhyk,bhzk->bhkyz", sgn_mat, sgn_mat)
...
class CheckerNet(nn.Module):
    ...
    def forward(self, x, query, key):
        ...
        if self.prob_attn:
            A1 = get_attn_adj(x, -1, K_reduce, self.super_p, self.jat_sgn)
        else:
            A1 = get_attn_adj(x, -1, key, self.super_p, self.jat_sgn)
        ...
        if self.checkertype==0:
            ...
            print("error")
            exit(0)
        h = torch.Tensor([0.]).to(out[0].device)
        for o in out:
            h = h + o[:,:,:Lk,:L]
        h = self.dropout(h)
        return h.contiguous()
...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if self.use_hattn and (not is_cross_attention) and (self.layer in self.olayers):
            # print("QAQ")
            # exit(0)
